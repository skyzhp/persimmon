	...
	for {
		...
		if tok.IsText() {
			if !inName {
				...
				if methodName != "" {
					return "", fmt.Errorf("Multiple method names"+" (\"%s\" and \"%s\")", methodName, tok.Text())
				}
		...
		if tok.Is(tokenMethodName) {
			if !tok.IsStart() {
				if !inName {
					return "", errors.New("Got </methodName> without" + " <methodName>")
				}
...
func getMethodData(p *xml.Decoder) ([]interface{}, *Fault, error) {
	...
	for {
		tok, err := getNextToken(p)
		if tok == nil {
			return nil, nil, errors.New("Unexpected end-of-file in" + " getMethodData()")
		} else if err != nil {
			return nil, nil, err
		}
...
func Unmarshal(r io.Reader) (string, []string, error, *Fault) {
	p := xml.NewDecoder(r)
	...
	v := reflect.ValueOf(extractParams)
	extLen := v.Len()
	extract := make([]string, extLen)
	for i := 0; i < extLen; i++ {
		extract[i] = v.Index(i).Interface().(string)
	}

	return methodName, extract, nil, fault
}
